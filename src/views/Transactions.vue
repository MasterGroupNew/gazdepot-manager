<template>
  <div class="space-y-6">
    ...
    <button
  @click="() => {
    console.log('isListening value:', speech.isListening.value);
  ...
  class="w-full h-16 text-white rounded-lg font-bold text-lg hover:opacity-90"
>
  {{ speech.isListening ? '🎤 ÉCOUTE...' : '🎤 PARLER' }}
</button>
      <p v-if="speech.transcript" class="text-sm text-blue-700 mt-2">
        {{ speech.transcript }}
      </p>
      <p v-if="speech.error" class="text-sm text-red-600 mt-2">
        {{ speech.error }}
      </p>
    </div>
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-5">
      <h2 class="font-semibold text-gray-800 mb-4">🔄 Nouvel échange client</h2>
    ...
    <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
      <table class="w-full text-sm">
        ...
        <tbody class="divide-y divide-gray-100">
          <tr v-for="tx in transactionsFiltrees" :key="tx.id" class="hover:bg-gray-50">
            <td class="px-5 py-3 text-gray-500">{{ tx.date }}</td>
            <td class="px-5 py-3">
              <span
                class="text-xs px-2 py-1 rounded-full font-medium"
                :class="tx.type === 'echange_client' ? 'bg-blue-50 text-blue-600' : 'bg-orange-50 text-orange-600'"
              >
...
const transactionsFiltrees = computed(() =>
  txStore.transactions.filter((tx) => {
    ...
    const matchSociete = filtreSociete.value ? tx.societe === filtreSociete.value : true
    return matchType && matchSociete
  }),
)

// Traiter la saisie vocale
watch(() => speech.transcript, (newTranscript) => {
  if (!newTranscript) return

  // Reconnaître les sociétés
  ...
  // Reconnaître les types de bouteille
  ...
  // Valider automatiquement si on entend "valider", "confirmer", "ok"
